	...
	public function makeWorkFlowLabel($status) {
		...
		switch($status) {
			...
			case WorkflowComponent::STATUS_APPROVED;	//承認待ち
				$html = "<span class='workflow-label label label-warning'>" . __d('net_commons', 'Approving') . "</span>";
				break;
			case WorkflowComponent::STATUS_IN_DRAFT:	//一時保存
				$html = "<span class='workflow-label label label-info'>" . __d('net_commons', 'In draft') . "</span>";
				break;
			case WorkflowComponent::STATUS_DISAPPROVED:	//差し戻し
				$html = "<span class='workflow-label label label-danger'>" . __d('net_commons', 'Disapproving') . "</span>";
				break;
		}
		return $html;
	}
